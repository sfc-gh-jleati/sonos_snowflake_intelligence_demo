...
CREATE OR REPLACE SEMANTIC VIEW SONOS_AI_DEMO.APP.SALES_SEMANTIC_VIEW
	tables (
		...
		SALES as SALES_FACT primary key (SALE_ID) with synonyms=('sales','purchases','transactions','orders') comment='Product sales transactions'
	)
	relationships (
	...
	dimensions (
		...
		PRODUCTS.CATEGORY_KEY as CATEGORY_KEY with synonyms=('category_id','product_category_id') comment='Product category identifier',
		...
		PRODUCT_CATEGORY_DIM.CATEGORY_NAME as CATEGORY_NAME with synonyms=('category','product category','product line') comment='Product category: Soundbars, Smart Speakers, Portable Speakers, Subwoofers, Amplifiers',
		PRODUCT_CATEGORY_DIM.VERTICAL as VERTICAL with synonyms=('vertical','product vertical') comment='Product vertical: Home Audio, Portable Audio, Accessories',
	...
	with extension (CA='{"tables":[{"name":"CUSTOMERS","dimensions":[{"name":"CUSTOMER_KEY"},{"name":"CUSTOMER_NAME"},{"name":"INDUSTRY","sample_values":["Tech Professional","Audiophile","Home Owner"]}]},{"name":"PRODUCTS","dimensions":[{"name":"CATEGORY_KEY","unique":false},{"name":"PRODUCT_KEY"},{"name":"PRODUCT_NAME","sample_values":["Sonos Arc","Sonos Beam (Gen 2)","Sonos Era 100","Sonos Era 300","Sonos Move 2","Sonos Roam","Sonos Sub (Gen 3)","Sonos Amp"]}]},{"name":"PRODUCT_CATEGORY_DIM","dimensions":[{"name":"CATEGORY_KEY"},{"name":"CATEGORY_NAME","sample_values":["Soundbars","Smart Speakers","Portable Speakers","Subwoofers","Amplifiers"]},{"name":"VERTICAL","sample_values":["Home Audio","Portable Audio","Accessories"]}]},{"name":"REGIONS","dimensions":[{"name":"REGION_KEY"},{"name":"REGION_NAME","sample_values":["North America","Europe","APAC","Latin America"]}]},{"name":"SALES","dimensions":[{"name":"CUSTOMER_KEY"},{"name":"PRODUCT_KEY"},{"name":"REGION_KEY"},{"name":"DATE","sample_values":["2024-01-01","2024-06-15","2025-07-01"]},{"name":"SALE_ID"},{"name":"SALE_MONTH"},{"name":"SALE_YEAR"}],"facts":[{"name":"AMOUNT"},{"name":"SALE_RECORD"},{"name":"UNITS"}],"metrics":[{"name":"AVERAGE_DEAL_SIZE"},{"name":"AVERAGE_UNITS_PER_SALE"},{"name":"TOTAL_DEALS"},{"name":"TOTAL_REVENUE"},{"name":"TOTAL_UNITS"}]}],"relationships":[{"name":"PRODUCT_TO_CATEGORY"},{"name":"SALES_TO_CUSTOMERS","relationship_type":"many_to_one"},{"name":"SALES_TO_PRODUCTS","relationship_type":"many_to_one"},{"name":"SALES_TO_REGIONS","relationship_type":"many_to_one"}],"verified_queries":[{"name":"Top products last 90 days North America","question":"Over the last 90 days in North America, which Sonos product led in units and revenue? Show top 5 with units, revenue, and MoM growth","sql":"WITH sales_90d AS (\\n  SELECT \\n    p.product_name,\\n    SUM(s.units) as total_units,\\n    SUM(s.amount) as total_revenue,\\n    MONTH(s.date) as sale_month\\n  FROM sales s\\n  JOIN products p ON s.product_key = p.product_key\\n  JOIN regions r ON s.region_key = r.region_key\\n  WHERE r.region_name = ''North America''\\n    AND s.date >= DATEADD(day, -90, CURRENT_DATE())\\n  GROUP BY p.product_name, MONTH(s.date)\\n)\\nSELECT \\n  product_name,\\n  SUM(total_units) as units,\\n  SUM(total_revenue) as revenue,\\n  (SUM(CASE WHEN sale_month = MONTH(CURRENT_DATE()) THEN total_revenue ELSE 0 END) / \\n   NULLIF(SUM(CASE WHEN sale_month = MONTH(DATEADD(month, -1, CURRENT_DATE())) THEN total_revenue ELSE 0 END), 0) - 1) * 100 as mom_growth_pct\\nFROM sales_90d\\nGROUP BY product_name\\nORDER BY revenue DESC\\nLIMIT 5","verified_by":"Sonos Demo","verified_at":1730000000}]}');
